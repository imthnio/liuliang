...
在 VPS 输入 `liuliang`，查看每个来源 IP 的运营商、城市、近24小时流量、近7天流量和最近活动时间。保留中文彩色表格，后台每120秒采样，自动开机启动。
...
2. **城市查询**：默认开启，回车即开。它会向 **ipwho.is** 发送连接客户端的 IP 查询国家、城市和运营商；输入 `n` 关闭，流量统计仍正常。

> 如果安装环境没有可交互的终端（比如某些网页终端里直接粘贴一行命令），安装器不会再报错退出：会自动使用检测到的代理端口、城市查询默认开启。只有**一个端口都没检测到**时才会停下来，这时按提示加上 `--ports` 参数重装即可。
...
```sh
curl -fsSL https://raw.githubusercontent.com/imthnio/liuliang/main/install.sh | sh -s -- --ports 443 --geo no
```

支持多个端口：`--ports 443,8443`。不加参数则自动使用检测到的端口、开启城市查询。表格新增「运营商」列：中国移动/电信/联通显示中文名，其余显示 ipwho.is 返回的 ISP 名称（如 BT 显示 BT）。
