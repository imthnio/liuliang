...
> 如果安装环境没有可交互的终端（比如某些网页终端里直接粘贴一行命令），安装器不会再报错退出：会自动使用检测到的代理端口、城市查询默认关闭。只有**一个端口都没检测到**时才会停下来，这时按提示加上 `--ports` 参数重装即可。
...
不需要城市信息时使用 `--geo no`。支持多个端口：`--ports 443,8443`。在没有交互终端的环境下也可以直接用这一行；不加参数则自动使用检测到的端口、关闭城市查询。
